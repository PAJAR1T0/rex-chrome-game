import { configValues, groundVelocity, ctx, sprite } from "./index";


export const drawCloud = ({h,w,x,y}: {h: number, w: number, x: number, y: number}) => {
...
export const drawGround = ({h,w,y}: {h: number, w: number, y: number}) => {
    configValues.groundx += groundVelocity;
    ctx.drawImage(sprite, configValues.groundx, y, w, h, 0, 123, w, h);

    if (configValues.groundx > w - 600) {
        ctx.drawImage(sprite, 0, y, w, h, w - configValues.groundx, 123, w, h);
        if ( configValues.groundx >= w) {
            configValues.groundx = 0;
        }
    }
};